...
Defines how LumaForge deploys the Provisioner Worker container image from Git through repository automation.
...
- **AND** the workflow SHALL publish the image to the configured container registry
...
- **AND** the workflow SHALL publish the image using the configured registry and manual channel tag inputs
...
#### Scenario: Manual channel tag is published
- **WHEN** the workflow is triggered manually with a channel tag
- **THEN** it SHALL publish that channel tag
- **AND** it SHALL also publish the immutable commit SHA tag
...
#### Scenario: Registry login uses secrets
- **WHEN** the workflow authenticates to the configured container registry
- **THEN** it SHALL read credentials from GitHub Actions secrets or built-in GitHub token context
...
- **AND** documentation SHALL list required repository secrets or variables
