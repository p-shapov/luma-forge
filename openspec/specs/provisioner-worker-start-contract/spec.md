...
- **AND** the request SHALL include a Workflow Preset with its Hugging Face API key requirement flag and declared model assets with asset id, display name, Hugging Face source coordinates, and ComfyUI-relative install path
- **AND** the request MUST NOT include Workflow Preset id, Workflow Preset version, workflow execution type, required base volume size, runtime contract reference, provisioner contract reference, resolved runtime image snapshot, resolved provisioner image snapshot, endpoint image fields, runtime manifest paths, endpoint runtime paths, Provider API Keys, Hugging Face API keys, or worker bearer tokens inside the JSON body
