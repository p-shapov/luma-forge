...
use crate::{
    domain::{
        placement::{
            ...
            RemoteGpuPlacementOption, RemotePlacementOptions,
        },
    ...
    I: ApiKeyIdentityProvider,
{
    ...
    fn delete_endpoint_and_template<'a>(
        ...
        Box::pin(async move {
            ...
            self.delete_rest(
                &format!("/templates/{}", response.template_id),
...
struct NetworkVolumeCreateBody {
...
#[derive(Debug, Deserialize)]
struct NetworkVolumeResponse {
    id: String,
}

#[derive(Debug, Serialize)]
struct PodCreateBody {
    #[serde(rename = "dataCenterId")]
    datacenter_id: String,
    #[serde(rename = "gpuTypeId")]
    gpu_id: String,
...
fn placement_graphql_request() -> GraphqlRequest<serde_json::Value> {
    GraphqlRequest {
...
fn network_volume_create_body(request: &CreateNetworkVolumeRequest) -> NetworkVolumeCreateBody {
...
fn provisioner_pod_create_body(request: &CreateProvisionerPodRequest) -> PodCreateBody {
...
fn endpoint_template_create_body(request: &CreateEndpointRequest) -> TemplateCreateBody {
    TemplateCreateBody {
        image_ref: request.image_ref.clone(),
        name: "luma-forge-endpoint-template".to_string(),
        ports: format!("{PROVISIONER_PORT}/http"),
        env: Vec::new(),
    }
}

fn endpoint_create_body(request: &CreateEndpointRequest, template_id: &str) -> EndpointCreateBody {
    EndpointCreateBody {
        ...
        name: "luma-forge-endpoint".to_string(),
        network_volume_id: request.network_volume_id.clone(),
...
fn map_send_error(error: reqwest::Error) -> RemoteWorkspaceError {
    ...
    fn endpoint_creation_serializes_template_before_endpoint_bodies() {
        ...
        assert_eq!(
            ...
            json!({
                ...
                "name": "luma-forge-endpoint-template",
                "ports": "8000/http",
                "env": []
            })
        );
