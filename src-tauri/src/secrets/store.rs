...
mod tests {
    ...
    #[test]
    fn provisioner_worker_tokens_use_separate_keyring_namespace() {
        let store = KeyringSecretStore::new("dev.lumaforge.test");

        assert_ne!(
            store.provider_service_name,
            store.provisioner_worker_service_name
        );
        assert_ne!(
            keyring_account(&GpuCloudProviderId::Runpod),
            provisioner_worker_account("018f6a40-0000-7000-8000-000000000001")
        );
    }

    #[test]
    fn provisioner_worker_token_debug_is_redacted() {
        let token =
            ProvisionerWorkerBearerToken::new("worker-token".to_string()).expect("valid token");

        assert_eq!(
            format!("{token:?}"),
            "ProvisionerWorkerBearerToken([REDACTED])"
        );
        assert!(!format!("{token:?}").contains("worker-token"));
    }

    #[test]
    fn rejects_blank_provisioner_worker_token() {
        assert_eq!(
            ProvisionerWorkerBearerToken::new(" ".to_string()).expect_err("blank token"),
            ProvisionerWorkerBearerTokenError
        );
    }
}
