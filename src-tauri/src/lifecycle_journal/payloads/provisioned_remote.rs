use crate::domain::lifecycle_operation::{
    LifecycleOperationPayload, ProvisionedRemoteLifecycleOperationPayload,
};

use super::super::LifecycleJournalError;

pub const RUNTIME_TYPE: &str = "provisioned_remote";

pub fn encode(
    payload: &ProvisionedRemoteLifecycleOperationPayload,
) -> Result<String, LifecycleJournalError> {
    serde_json::to_string(&LifecycleOperationPayload::ProvisionedRemote(
        payload.clone(),
    ))
    .map_err(|_| LifecycleJournalError::Corrupt)
}

pub fn decode(payload_json: &str) -> Result<LifecycleOperationPayload, LifecycleJournalError> {
    let payload: LifecycleOperationPayload =
        serde_json::from_str(payload_json).map_err(|_| LifecycleJournalError::Corrupt)?;

    match payload {
        LifecycleOperationPayload::ProvisionedRemote(_) => Ok(payload),
    }
}
