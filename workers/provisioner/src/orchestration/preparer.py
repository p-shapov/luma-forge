...
class Provisioner:
    ...
    def prepare(self, request: StartRequest, progress: ProgressCallback) -> None:
    ...
    def _download_assets(
        ...
        for index, asset in enumerate(assets, start=1):
            ...
            self.downloader.download(
                asset,
                target,
                download_inactivity_timeout_seconds=self.config.download_inactivity_timeout_seconds,
    ...
    def _prepare_workspace_paths(self, workspace_root: Path) -> None:
        for directory in [
