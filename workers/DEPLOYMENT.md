...
RunPod Endpoint Worker deployment is driven by [`promote-runtime-contract/`](./promote-runtime-contract/). That module owns endpoint contract YAML, schema validation, endpoint image build metadata resolution, and Endpoint Contracts promotion. [`promote-provisioner-contract/`](./promote-provisioner-contract/) owns Provisioner Contracts promotion.
...
Manual provisioner releases publish a provisioner image, then automatically propose Provisioner Contracts promotion under `luma-forge-provisioner`. The workflow resolves the next Provisioner Contracts patch version from `bundled/provisioner-contracts.json`, for example `1.0.0` to `1.0.1`, before provisioner validation, image build, or publication.
...
After publishing a validated provisioner image, the workflow opens a reviewed Provisioner Contracts promotion PR that appends the selected provisioner contract id/version revision in `bundled/provisioner-contracts.json` with a digest-pinned provisioner image ref. The same PR updates `bundled/workflow-catalog.json` so Workflow Presets using that provisioner contract id point at the new revision.

After publishing a validated endpoint image, the workflow opens a reviewed Endpoint Contracts promotion PR that appends the selected endpoint contract id/version revision in `bundled/endpoint-contracts.json` with a digest-pinned endpoint image ref. The same PR updates `bundled/workflow-catalog.json` so the Workflow Preset whose id matches the endpoint contract id points at the new revision.

Catalog promotion PRs are path-guarded. Provisioner PRs may change only `bundled/provisioner-contracts.json` and `bundled/workflow-catalog.json`; endpoint PRs may change only `bundled/endpoint-contracts.json` and `bundled/workflow-catalog.json`.
